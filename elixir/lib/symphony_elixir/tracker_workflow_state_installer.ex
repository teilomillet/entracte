defmodule SymphonyElixir.TrackerWorkflowStateInstaller do
  @moduledoc """
  Loads runner config and asks the configured tracker adapter to install workflow states.
  """
  ...
  @type deps :: %{
          ...
          required(:install_workflow_states) => (keyword() -> {:ok, [WorkflowStateInstallation.t()]} | {:error, term()})
        }

  @spec install(keyword(), deps()) :: {:ok, [WorkflowStateInstallation.t()]} | {:error, term()}
  def install(opts \\ [], deps \\ runtime_deps()) when is_list(opts) and is_map(deps) do
    ...
    with :ok <- load_env(opts, workflow_path, deps),
      ...
      deps.install_workflow_states.(opts)
    end
  end
  ...
  defp runtime_deps do
    %{
      ...
      install_workflow_states: &Tracker.install_workflow_states/1
    }
  end
